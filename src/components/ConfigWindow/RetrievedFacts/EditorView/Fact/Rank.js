...
export default class Rank extends Component {
    ...
    render() {
        ...
        if (fact.measure.length && fact.breakdown.length) {
            ...
            if (!newFact.focus.length || preAggregate !== currentAggregate || preMeasure !== currentMeasure || preSubspace.toString() !== currentSubspace.toString() || preFilteredData.toString() !== currentFilteredData.toString()) {
                ...
                newOrder.forEach((d, i) => {
                    if (i < 3) {
                        newFact.focus.push({ field: fact.breakdown[0], value: d })
                    }
                });
                // newFact.focus = [
                //     { field: fact.breakdown[0], value: newOrder[0] },
                //     { field: fact.breakdown[0], value: newOrder[1] },
                //     { field: fact.breakdown[0], value: newOrder[2] }
                // ]
        ...
        let modalPosition;
        if (document.getElementById('add-subspace-rank')) {
            modalPosition = document.getElementById('add-subspace-rank').getBoundingClientRect()
        }
        ...
        return (
            <div className="config-panel">
                ...
                <Row className={fact.subspace.length === 0 ? 'shelf' : ''}>
                    ...
                    <Col span={16}>
                        <Button id='add-subspace-rank' size='small' onClick={showModal}>
                            +
                        </Button>
                    </Col>
                </Row>
                ...
                {fact.focus.map((key, i) => <Row className={i === 0 ? 'shelf' : ''} key={i} >
                    ...
                    <Col span={16}>
                        {/* <Col span={2}></Col> */}
                        <Row style={{marginBottom: '0', border: '0'}}>
                        <Col span={20} style={{ overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', paddingLeft: '7px', display: 'flex', alignItems: 'center' }}>{`No.${i + 1} (${key.value})`}</Col>
