...
export default class Categorization extends Component {
    ...
    render() {
        let { handleChartChange, getFieldValue, isDisabled, handleFilterChange, onRadioChange, removeFilter, handleSubOk, handleSubCancel, handleBreakdownChange, showModal, handleFocusChange, onFocusClick, removeFocus, onFocusBlur } = this.props;
        ...
        let modalPosition;
        if (document.getElementById('add-subspace-categorization')) {
            modalPosition = document.getElementById('add-subspace-categorization').getBoundingClientRect()
        }
        ...
        return (
            <div className="config-panel">
                ...
                <Row className={fact.subspace.length === 0 ? 'shelf' : ''}>
                    ...
                    <Col span={16}>
                        <Button id='add-subspace-categorization' ref='addSubBtn' size='small' onClick={showModal}>
                            +
                        </Button>
                    </Col>
                </Row>
