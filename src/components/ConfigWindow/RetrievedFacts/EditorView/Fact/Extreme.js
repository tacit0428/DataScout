...
export default class Extreme extends Component {
    ...
    componentDidMount() {
        ...
        if (fact.measure.length && fact.breakdown.length) {
            ...
            this.breakdownValueList = newbreakdownValueList
            ...
            if (!newFact.focus.length) {
                ...
                this.focusDefaultValue = `${newFact.focus[0].extremeFocus} (${newFact.focus[0].value})`
                this.props.updateFact(newFact, this.props.factIndex, 'extreme')
            } else {
                let focus = newFact.focus[0]
                if (!focus.extremeFocus) {
                    if (focus.value == max[fact.breakdown[0]]) {
                        focus.extremeFocus = 'max'
                        newFact.focus = [focus]
                    } else if (focus.value == min[fact.breakdown[0]]) {
                        focus.extremeFocus = 'min'
                        newFact.focus = [focus]
                    } else {
                        newFact.focus = [{
                            field: fact.breakdown[0],
    ...
    render() {
        ...
        if (fact.focus.length) {
            this.focusDefaultValue = `${fact.focus[0].extremeFocus} (${fact.focus[0].value})`
        }
        ...
        return (
            <div className="config-panel">
                ...
                <Row className="shelf">
                    <Col span={8} className="channelName">Focus</Col>
                    <Col span={16}>
                        <Select className="select-box" defaultValue={this.focusDefaultValue} value={this.focusDefaultValue} size='small' onChange={handleFocusChange}>
                            {this.breakdownValueList.map((key) => <Option key={key.value} value={key.value}>{key.displayName}</Option>)}
                        </Select>
                    </Col>
                </Row>
            </div>
...
//                     extremeValue: max[fact.measure[0].field]
//                 }]
//                 focusDefaultValue = `${newFact.focus[0].extremeFocus} (${newFact.focus[0].value})`
