...
export default class Value extends Component {
    ...
    render() {
        let { handleChartChange, getFieldValue, isDisabled, handleMeasureChange, handleAGGChange, handleFilterChange, handleBreakdownChange, handleFocusChange, onFocusBlur, onRadioChange, removeFilter, handleSubOk, handleSubCancel, showModal } = this.props;
        ...
        let modalPosition;
        if (document.getElementById('add-subspace-value')) {
            modalPosition = document.getElementById('add-subspace-value').getBoundingClientRect()
        }
        ...
        return (
            <div className="config-panel">
                ...
                <Row className={fact.subspace.length === 0 ? 'shelf' : ''}>
                    ...
                    <Col span={16} >
                        <Button id='add-subspace-value' size='small' onClick={showModal}>
                            +
                        </Button>
                    </Col>
                </Row>
                ...
                <Row className="shelf">
                    ...
                    <Col span={16}>
                        <Select className="select-box" defaultValue={'please select'} size='small' onChange={handleFocusChange}>
                            {breakdownValueList.map((key) => <Option key={key} value={key}>{key}</Option>)}
                        </Select>
                    </Col>
                </Row> */}
