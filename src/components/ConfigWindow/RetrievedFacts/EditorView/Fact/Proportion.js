...
export default class Proportion extends Component {
    ...
    render() {
        ...
        if (document.getElementById('add-subspace-propotion')) {
            modalPosition = document.getElementById('add-subspace-propotion').getBoundingClientRect()
        }
        ...
        if (fact.focus.length) {
            for (const focus of fact.focus) {
                if (breakdownValueList.includes(focus.value)) {
                    curFocus = focus.value
                    break
                }
            }
        }
        ...
        return (
            <div className="config-panel">
                ...
                <Row className={fact.subspace.length === 0 ? 'shelf' : ''}>
                    ...
                    <Col span={16} >
                        <Button id='add-subspace-propotion' size='small' onClick={showModal}>
                            +
                        </Button>
                    </Col>
                </Row>
                ...
                <Row className="shelf">
                    ...
                    <Col span={16}>
                        <Select className="select-box" defaultValue={curFocus} value={curFocus} size='small' onChange={handleFocusChange}>
                            {breakdownValueList[0] !== undefined && breakdownValueList.map((key) => <Option key={key} value={key}>{key}</Option>)}
                        </Select>
                    </Col>
                </Row>
