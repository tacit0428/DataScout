...
const draw = (props) => {
    const chartName = 'vis-scatterplot-'+props.uuid
    ...
    const buffer = 0.05; 
    // X channel
    const xExtent = d3.extent(data.map(d => parseFloat(d[encoding.x.field])));
    const xScale = d3.scaleLinear()
        // .domain([0, d3.max(data.map(d => parseFloat(d[encoding.x.field])))])//d3.extent(data.map(d=>d[encoding.x.field]))
        .domain([
            xExtent[0] - buffer * (xExtent[1] - xExtent[0]),
            xExtent[1] + buffer * (xExtent[1] - xExtent[0])
        ])
        .range([0, chartWidth])
        .nice();
        
    // Y channel
    const yExtent = d3.extent(data.map(d => parseFloat(d[encoding.y.field])));
    const yScale = d3.scaleLinear()
        // .domain([0, d3.max(data.map(d => parseFloat(d[encoding.y.field])))])//d3.extent(data.map(d=>d[encoding.y.field]))
        .domain([
            yExtent[0] - buffer * (yExtent[1] - yExtent[0]),
            yExtent[1] + buffer * (yExtent[1] - yExtent[0])
        ])
        .range([chartHeight, 0])
        .nice();
